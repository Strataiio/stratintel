...
export default function Dashboard() {
  ...
  async function createNew() {
    ...
    if (error) { showToast('Error creating form: ' + error.message); return }
    navigate(`/builder/${id}`)
  }
      ...
      <div className="main">
        <div className="fade-up">
          ...
          <div className="stats-row fade-up-1">
            <div className="stat-cell"><div className="stat-label">Total Forms</div><div className="stat-value">{forms.length}</div></div>
            <div className="stat-cell"><div className="stat-label">Live</div><div className="stat-value" style={{ color: 'var(--success)' }}>{published}</div></div>
            <div className="stat-cell"><div className="stat-label">Responses</div><div className="stat-value" style={{ color: 'var(--accent2)' }}>{totalResps}</div></div>
          </div>
          {loading ? (
            ...
            <div className="empty fade-up-2">
              ...
              <p style={{ fontSize: 14, marginBottom: 16 }}>No forms yet.</p>
            ...
            <div className="forms-grid fade-up-2">
              {forms.map(f => (
                <div key={f.id} className="card card-hover" style={{ display: 'flex', flexDirection: 'column', gap: 12 }} onClick={() => navigate(`/builder/${f.id}`)}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8 }}>
                    <div style={{ fontFamily: 'var(--font-head)', fontSize: 15, fontWeight: 600, lineHeight: 1.3 }}>
                      {f.title || 'Untitled Form'}
                    </div>
                    <span className={`badge ${f.published ? 'badge-live' : 'badge-draft'}`}>{f.published ? '● Live' : 'Draft'}</span>
                  </div>
                  {f.description && <p style={{ fontSize: 12, color: 'var(--text3)', lineHeight: 1.55 }}>{f.description.slice(0, 90)}{f.description.length > 90 ? '…' : ''}</p>}
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11, color: 'var(--text3)' }}>
                    <span>{f.fields?.length ?? 0} fields</span><span>·</span>
                    <span>{respCounts[f.id] ?? 0} responses</span><span>·</span>
                    <span>{formatDate(f.created_at)}</span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 'auto', paddingTop: 10, borderTop: '1px solid var(--border)' }} onClick={e => e.stopPropagation()}>
                    ...
                    <div style={{ marginLeft: 'auto' }}>
                      <button className="btn-icon" style={{ color: 'var(--danger)', borderColor: 'rgba(255,118,117,0.2)' }} onClick={e => deleteForm(e, f.id)}><IconTrash /></button>
                    </div>
                  </div>
                </div>
              ))}
