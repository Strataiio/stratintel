...
export default function Responses() {
  ...
  function downloadReport(resp: FormResponse) {
    const lines = [`StratIntel AI Analysis Report`, `${'='.repeat(42)}`,
      `Form: ${resp.form_title}`, `Date: ${formatDateTime(resp.created_at)}`,
      `Respondent: ${resp.respondent_email || '—'}`, ``, `FORM RESPONSES`, `${'─'.repeat(30)}`
    ]
    if (form) {
      form.fields.forEach(f => {
        lines.push(`${f.label}: ${resp.response_data[f.id] || '—'}`)
      })
    }
            ...
            <div className="fade-up" style={{ maxWidth: 720 }}>
              ...
              <div className="card" style={{ marginBottom: 14 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                  <div>
                    <div style={{ fontWeight: 500, fontSize: 14 }}>{selected.respondent_email || 'Anonymous'}</div>
                    <div style={{ fontSize: 12, color: 'var(--text3)' }}>{formatDateTime(selected.created_at)}</div>
                  </div>
                  <button className="btn btn-sm" onClick={() => downloadReport(selected)}><IconDownload /> Download</button>
                </div>
                <div style={{ borderTop: '1px solid var(--border)', paddingTop: 14 }}>
                  <div style={{ fontSize: 11, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 12 }}>Form Answers</div>
                  {form?.fields.map(f => (
                    <div key={f.id} style={{ marginBottom: 12 }}>
                      <div style={{ fontSize: 11, color: 'var(--text2)', marginBottom: 3 }}>{f.label}</div>
                      <div style={{ fontSize: 14, color: 'var(--text)' }}>{selected.response_data[f.id] || <span style={{ color: 'var(--text3)' }}>—</span>}</div>
                    </div>
                  ))}
                </div>
              </div>
                  ...
                  <tbody>
                    {responses.map((r, i) => (
                      <tr key={r.id} style={{ cursor: 'pointer' }} onClick={() => setSelected(r)}>
                        <td style={{ color: 'var(--text3)', fontSize: 12 }}>{responses.length - i}</td>
                        <td>{r.respondent_email || <span style={{ color: 'var(--text3)' }}>Anonymous</span>}</td>
                        <td style={{ fontSize: 12 }}>{formatDateTime(r.created_at)}</td>
                        <td style={{ fontSize: 12, color: 'var(--text3)', maxWidth: 220 }}>
